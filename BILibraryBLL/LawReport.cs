...
namespace BILibraryBLL
{
    public class LawReport
    {
        ...
        public DataTable LawReportMth(string offcode)
        {
            ...
            string sql = @"select * from (select TRANS_Short_month(budget_month_desc) as month, SUM(LAW_QTY) AS LAW_QTY, SUM(TARGET_QTY) AS TARGET_QTY,SUM(LAW_AMT) AS LAW_AMT,SUM(TARGET_AMT) AS TARGET_AMT,
...
where offcode = "+ offcode + " group by budget_month_desc,time_id ";
            sql += @" union all
select 'รวม', SUM(LAW_QTY) AS LAW_QTY, SUM(TARGET_QTY) AS TARGET_QTY,SUM(LAW_AMT) AS LAW_AMT,SUM(TARGET_AMT) AS TARGET_AMT,
