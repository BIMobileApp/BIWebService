...
namespace BILibraryBLL
{
    public class TaxRealtime
    {
        ...
        public DataTable FollowPayTaxRealtime(string month)
        {
        ...
        public DataTable TaxRealtimeFreezone(string month) {
                          ...
                          FROM (SELECT *
                                  FROM (select b.day_thai as time_id
                                        ...
                                        where a.dim_data_date_id = b.time_id
                                              ...
                                              and b.month_cd = 1 
                                              and b.budget_year = 2560
                                              group by b.day_thai ,c.import_status_sub ,c.import_status_code
                                         ) PIVOT(sum(tax) as tax FOR cd in('1' AS A_in
                                                                          ,'2' AS A_out
                                                                          ,'3' AS A_free
                                                                          ,'4' AS A_outpay))
